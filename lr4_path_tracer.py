...
Программа строит изображение треугольной сцены методом path tracing
с учетом диффузного и зеркального отражения, area-light источников,
anti-aliasing, Russian roulette и простого GUI.
"""
...
def random_in_unit_square(rng: np.random.Generator) -> Tuple[float, float]:
    # Равномерная выборка внутри пикселя для anti-aliasing.
...
class Camera:
    ...
    def generate_ray(self, x: float, y: float, width: int, height: int) -> Ray:
        # Луч камеры через точку изображения; point camera без толщины линзы.
...
def intersect_scene(scene: Scene, ray: Ray) -> Optional[Hit]:
    ...
    if np.dot(n, d) > 0.0:
        n = -n
    return Hit(t=best_t, position=pos, normal=n, material_id=int(scene.tri_mat_ids[tri_id]), tri_id=tri_id)
...
def choose_event(material: Material, rng: np.random.Generator) -> str:
    # Выбор события по значимости: вероятность пропорциональна средней энергии зеркальной и диффузной части.
    kd = float(np.mean(material.diffuse))
    ks = float(np.mean(material.mirror))
    s = kd + ks
    if s <= 1e-12:
        return "absorb"
...
def direct_light(scene: Scene, hit: Hit, rng: np.random.Generator) -> np.ndarray:
    # Оценка прямого освещения от одного случайного area-light: L = Le * f_r * G / p(light) / p_A.
    ...
    brdf = scene.materials[hit.material_id].diffuse / math.pi
    geom = cos_surface * cos_light / r2
    return light_mat.emission * brdf * geom / max(p_light * p_area, 1e-12)


def trace_path(scene: Scene, ray: Ray, rng: np.random.Generator, max_depth: int, rr_start: int) -> np.ndarray:
    # Path tracing: L = sum beta * Le + beta * direct + beta * indirect, где beta — накопленный throughput.
    ...
    for depth in range(max_depth):
        hit = intersect_scene(scene, ray)
        if hit is None:
            L += beta * scene.background
            break

        mat = scene.materials[hit.material_id]
        if np.any(mat.emission > 0.0):
            # TODO:
            # 1) если предыдущее событие было зеркально или прямой взгляд, то это работает
            # когда предыдущее было диффузное, то это работать не будет
            # 2) Надо реализовать Кук - Торренс как второй метод
            # optional: распараллелить
            # ДЛЯ 5ЛР:
            # в чем разница синтезированного изображения и обычного по отношению к фильтрации
            # билатеральная фильтрация, гауссовы фильтры, много(?)теральная фильтрация
            # медианный фильтр
            # какие проблемы при приведении к 0-255
            L += beta * mat.emission

        if np.any(mat.diffuse > 0.0):
            L += beta * direct_light(scene, hit, rng)

        event = choose_event(mat, rng)
        if event == "absorb":
            break

        if event == "diffuse":
            # Lambert: f_r = rho_d / pi, p = cos(theta) / pi => beta *= rho_d.
            new_dir = sample_cosine_hemisphere(hit.normal, rng)
            beta = beta * mat.diffuse
            ...
            beta = beta * mat.mirror
            new_origin = hit.position + hit.normal * EPS * 4.0
            ray = Ray(new_origin, new_dir)

        if depth >= rr_start:
            # Russian roulette: продолжаем путь с вероятностью p, а вклад делим на p для несмещенности оценки.
...
def add_quad(tris: List[Triangle], a: np.ndarray, b: np.ndarray, c: np.ndarray, d: np.ndarray, material_id: int) -> None:
    # Четырехугольник раскладываем на два треугольника.
    tris.append(Triangle(a, b, c, material_id))
    tris.append(Triangle(a, c, d, material_id))
...
def build_cornell_scene(
    ...
    add_quad(tris, make_np3(x0, y0, z1), make_np3(x1, y0, z1), make_np3(x1, y0, z0), make_np3(x0, y0, z0), 0)
    add_quad(tris, make_np3(x0, y1, z0), make_np3(x1, y1, z0), make_np3(x1, y1, z1), make_np3(x0, y1, z1), 0)
    add_quad(tris, make_np3(x0, y0, z0), make_np3(x0, y0, z1), make_np3(x0, y1, z1), make_np3(x0, y1, z0), 1)
    add_quad(tris, make_np3(x1, y0, z1), make_np3(x1, y0, z0), make_np3(x1, y1, z0), make_np3(x1, y1, z1), 2)
    add_quad(tris, make_np3(x0, y0, z0), make_np3(x1, y0, z0), make_np3(x1, y1, z0), make_np3(x0, y1, z0), 0)
    ...
    add_quad(tris, p000, p100, p110, p010, 3)
    add_quad(tris, p001, p101, p111, p011, 3)
    add_quad(tris, p000, p100, p101, p001, 3)
    add_quad(tris, p100, p110, p111, p101, 3)
    add_quad(tris, p110, p010, p011, p111, 3)
    add_quad(tris, p010, p000, p001, p011, 3)
    ...
    add_quad(tris, q000, q100, q110, q010, 4)
    add_quad(tris, q001, q101, q111, q011, 4)
    add_quad(tris, q000, q100, q101, q001, 4)
    add_quad(tris, q100, q110, q111, q101, 4)
    add_quad(tris, q110, q010, q011, q111, 4)
    add_quad(tris, q010, q000, q001, q011, 4)

    if add_mesh:
        mmid = 3 if mesh_material_id is None else mesh_material_id
        for tri in add_mesh:
            tris.append(Triangle(tri.v0.copy(), tri.v1.copy(), tri.v2.copy(), mmid))
...
def load_obj_triangles(path: str, scale: float = 1.0, offset: Tuple[float, float, float] = (0.0, 0.0, 0.0)) -> List[Triangle]:
    # OBJ-loader для вершин v и граней f; полигоны fan-triangulation -> треугольники.
...
class PathTracer:
    ...
    def render(self, progress_cb=None, preview_cb=None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for y in range(h):
            ...
            for x in range(w):
                ...
                for _ in range(spp):
                    ...
                    pixel += trace_path(self.scene, ray, rng, self.params.max_depth, self.params.rr_start)
                hdr[y, x] = pixel / max(1, spp)

            frac = (y + 1) / h
            if progress_cb is not None:
                progress_cb(frac)
            if preview_cb is not None and (((y + 1) % self.params.preview_every_rows == 0) or (y + 1 == h)):
...
class App:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(lf, text="Нормировка").grid(row=9, column=0, sticky="w", pady=2)
        norm_box = ttk.Combobox(lf, textvariable=self.norm_var, values=["max", "mean05"], width=10, state="readonly")
        norm_box.grid(row=9, column=1, sticky="ew", pady=2, padx=5)
    ...
    def start_render(self) -> None:
        ...
        def worker() -> None:
            try:
                ...
                hdr, rgb8 = tracer.render(progress_cb=progress_cb, preview_cb=preview_cb)
                elapsed = time.time() - start
                self.progress_queue.put(("done", (hdr, rgb8, elapsed)))
        ...
        try:
            while True:
                ...
                if kind == "progress":
                    ...
                    hdr, rgb8, elapsed = data
                    self.last_hdr = hdr
                    self.last_rgb8 = rgb8
                    self.show_image(rgb8)
                    self.pb["value"] = 100.0
